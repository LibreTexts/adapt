<template>
  <div>
    ...
    >
      ...
      <b-alert v-if="submissionsAtRisk" variant="danger" show>
        <b-form-checkbox
      ...
      <b-alert v-else variant="info" show>
        This assignment is closed and has no student submissions for this Learning Tree, so updating won't affect
        any students.
      </b-alert>
      <template #modal-footer>
        <b-button size="sm"
...
export default {
  ...
  props: {
    ...
    // EK: mirrors AssignmentQuestionLearningTree::updateRisksRealStudentSubmissions() -
    // whether the assignment is open or already has real student
    // submissions for this tree. When false, the update can't affect any
    // students, so we skip the scary checkbox in favor of a plain confirm.
    // The backend re-checks this itself before honoring
    // understandStudentSubmissionsRemoved, so a stale/wrong value here can't
    // let a real wipe through without confirmation.
    submissionsAtRisk: {
      type: Boolean,
      default: true
    }
  ...
  methods: {
    ...
    async updateTheLearningTreeRevision () {
      if (this.submissionsAtRisk && !this.understandStudentSubmissionsRemoved) {
      ...
      try {
        const { data } = await axios.patch(`/api/assignments/${this.assignmentId}/learning-tree/${this.learningTreeId}/update-to-latest-revision`,
          // EK: when there's nothing at risk there's no checkbox to check, so
          // there's nothing meaningful to send here either - the backend
          // doesn't require this flag to be true unless it independently
          // agrees submissions are actually at risk.
          { understand_student_submissions_removed: this.submissionsAtRisk ? this.understandStudentSubmissionsRemoved : true })
        if (data.type === 'error') {
