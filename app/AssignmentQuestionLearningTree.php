...
class AssignmentQuestionLearningTree extends Model
{
    ...
    {
        // EK: must keep html/blockarr, not just blocks - Flowy.js's
        // flowy.import() renders purely from output.html (sets the
        // canvas's innerHTML directly) and walks output.blockarr for
        // block layout/position; it never reads output.blocks at all
        // (that's only used by the reverse direction, flowy.output(),
        // to serialize the DOM back into JSON). blocks-only was
        // sufficient for learningTreeNeedsUpdate()'s diffing below, which
        // only ever reads the ['blocks'] key from either side, but it's
        // insufficient to actually render the snapshot on canvas -
        // importing a blocks-only snapshot set the canvas's innerHTML to
        // the literal string "undefined" (output.html being undefined)
        // and then threw trying to read output.blockarr.length. Storing
        // the full decoded live tree, with only blocks[].data mutated
        // below, keeps learningTreeNeedsUpdate() unaffected while making
        // the snapshot actually renderable.
        $decoded = json_decode($learningTree->learning_tree, true) ?? [];
                                                      ...
                                                      AssignmentSyncQuestion $assignmentSyncQuestion): bool
    {
        ...
        return $this->hasRealStudentWorkOnTree($assignment, $learningTree, $assignmentSyncQuestion);
    }

    /**
     * Whether autoUpdateEligibleAssignmentSnapshots() can safely auto-refresh
     * this assignment's tree snapshot without instructor confirmation -
     * matches QuestionController::store()'s per-question notify auto-update
     * precedent exactly: does this assignment's course have any real
     * student enrolled at all, regardless of whether that student has
     * touched this particular tree yet. "Real" means fake_student=0 OR
     * formative_student=1 - the same enrollments/users join and where
     * clause used there, not a reinterpretation of it.
     *
     * Deliberately does NOT look at learning_tree_node_submissions or the
     * root question's own submissions the way updateRisksRealStudentSubmissions()
     * (the manual button's check) does - a course-wide enrollment check is
     * coarser by design, matching the existing question-revision precedent
     * rather than a tree-specific one.
     *
     * @param Assignment $assignment
     * @return bool
     */
    public function autoUpdateRisksRealStudents(Assignment $assignment): bool
    {
    ...
    /**
     * Used by updateRisksRealStudentSubmissions() (the manual "Update to
     * Latest Revision" button's check) - whether real (non-fake) student
     * work already exists on this tree in this assignment: the root node's
     * own submission, or any other node's learning_tree_node_submissions
     * row. Deliberately ignores fake students on both counts, same as every
     * other "does this affect real students" check in this feature (see
     * AssignmentSyncQuestion::questionHasSomeTypeOfRealStudentSubmission()).
     * Not used by the automatic path - see autoUpdateRisksRealStudents()
     * above, which matches QuestionController::store()'s coarser course-wide
     * enrollment check instead.
     *
     * @param Assignment $assignment
     * @param LearningTree $learningTree
     * @param AssignmentSyncQuestion $assignmentSyncQuestion
     * @return bool
     */
    private function hasRealStudentWorkOnTree(Assignment             $assignment,
                                              LearningTree           $learningTree,
                                              AssignmentSyncQuestion $assignmentSyncQuestion): bool
    {
        try {
    ...
    /**
     * After a structural edit to a Learning Tree
     * (LearningTreeController::updateLearningTree()), automatically resyncs
     * this tree's snapshot for every assignment where that's safe to do
     * without instructor confirmation: the assignment's course has
     * auto_update_question_revisions=1, AND the course has no real student
     * enrolled at all (autoUpdateRisksRealStudents() below) - matching
     * QuestionController::store()'s per-question notify auto-update
     * precedent exactly, including its course-wide, enrollment-based
     * (rather than tree-specific, submission-based) definition of "safe."
     *
     * Assignments that don't qualify are left alone - learningTreeNeedsUpdate()
     * will keep flagging them as stale until an instructor manually updates
     * them (or explicitly confirms the submission-wipe via the normal
     * "Update to Latest Revision" flow).
     *
     * @param LearningTree $learningTree
     * @param AssignmentSyncQuestion $assignmentSyncQuestion
     * @return array assignment ids that were auto-updated
     */
    public function autoUpdateEligibleAssignmentSnapshots(LearningTree $learningTree, AssignmentSyncQuestion $assignmentSyncQuestion): array
    {
        $candidate_assignment_ids = DB::table('assignment_question_learning_tree')
            ->join('assignment_question', 'assignment_question_learning_tree.assignment_question_id', '=', 'assignment_question.id')
        ...
        foreach ($candidate_assignment_ids as $candidate_assignment_id) {
            $assignment = Assignment::find($candidate_assignment_id);
            if (!$assignment) {
                continue;
            }
            if ($this->autoUpdateRisksRealStudents($assignment)) {
                // real student work could be affected - leave this one for
                // an instructor to explicitly confirm instead
                continue;
            }
            try {
                $this->updateToLatestRevision($assignment, $learningTree, $assignmentSyncQuestion);
                $updated_assignment_ids[] = $candidate_assignment_id;
            } catch (Exception $e) {
                // something unexpected about this specific assignment (e.g.
                // its assignment_question row is missing) - leave it alone
                // rather than fail the whole tree save over one bad row
                continue;
            }
        }
    ...
    {
        // EK: same html/blockarr requirement as buildLearningTreeSnapshot() -
        // flowy.import() renders from output.html/output.blockarr, never
        // output.blocks, so re-encoding blocks-only here silently strips
        // those back out of a snapshot that previously had them, breaking
        // rendering the next time anyone opens the tree inside this
        // assignment. This path (propagate / notify's auto-update) is
        // separate from buildLearningTreeSnapshot() and was missed when
        // that one was fixed.
        ...
        if ($changed) {
            ...
            DB::table('assignment_question_learning_tree')
                ->where('id', $assignment_question_learning_tree->id)
                ->update([
                    'learning_tree' => json_encode($decoded),
                    'updated_at' => Carbon::now()
                ]);
        }
        return $changed;
