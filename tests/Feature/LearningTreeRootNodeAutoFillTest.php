...
class LearningTreeRootNodeAutoFillTest extends TestCase
{
    ...
    public function it_does_not_overwrite_existing_tags_on_the_tree()
    {
        ...
        // EK: the real frontend always sends the tree's CURRENT tags on
        // every save (learningTreeForm.tags is populated from the tree's
        // existing state before submit) - it never omits the field to
        // mean "leave tags alone". Omitting 'tags' here would make
        // addTags(null ?: []) wipe the existing tag via cleanUpTags()
        // before the auto-fill check even runs, which isn't what this
        // test is meant to exercise.
        $data = array_merge($this->base_update_data, ['tags' => [$existing_tag->tag]]);

        $this->actingAs($this->user)
            ->postJson("/api/learning-trees/info/{$this->learningTree->id}", $data)
            ->assertJson(['type' => 'success']);

        // EK: don't assert on $existing_tag->id directly - addTags() always
        // runs cleanUpTags() first (deleting the old learning_tree_tag row,
        // and the underlying tags row too if nothing else references it)
        // BEFORE re-adding the tags from this same request. Re-saving the
        // same tag name in the same request therefore recreates the tags
        // row under a NEW id - the tag NAME is preserved, not the row ID.
        // Assert on the name via a join instead.
        $this->assertDatabaseHas('tags', ['tag' => 'tree already has this']);
        $tree_tag_names = DB::table('learning_tree_tag')
            ->join('tags', 'learning_tree_tag.tag_id', '=', 'tags.id')
            ->where('learning_tree_id', $this->learningTree->id)
            ->pluck('tag');
        $this->assertEquals(['tree already has this'], $tree_tag_names->toArray());
